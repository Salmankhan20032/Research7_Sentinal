...
\newcommand{\orcidicon}[1]{\href{https://orcid.org/#1}{\mbox{\begin{tikzpicture}[baseline=-0.35em,scale=0.08]
...
\begin{align}
s_{t+1}^{\mathrm{live}} &= \psi_{\mathrm{live}}(s_t^{\mathrm{live}}, c_t, T_t), \\
s_{t+1}^{\mathrm{decoy}, k} &= \psi_{\mathrm{decoy}}^{(k)}(s_t^{\mathrm{decoy}, k}, c_t),
\end{align}
...
\begin{equation}
\psi_{\mathrm{live}}(s_t^{\mathrm{live}}, c_t, T_t) = 
\begin{cases}
\delta(s_t^{\mathrm{live}}, c_t), & \text{if } \mathsf{VerifyCap}(c_t, T_t, K_{\mathrm{epoch}}) = \mathsf{True},\\
...
\phi(S_t) &= \operatorname{Concat}\big( \Phi_1(S_t), \Phi_2(S_t), \dots, \Phi_{19}(S_t) \big) \in \mathbb{R}^{152}.
\end{align}
...
\begin{equation}
 s_t = 1-P(y=\text{normal}\mid \phi(S_t)).
\end{equation}
The response is
\begin{equation}
 D(s_t)=
 \begin{cases}
 \text{approve}, & s_t < \tau_{\mathrm{clear}},\\
 \text{deceive}, & \tau_{\mathrm{clear}} \le s_t < \tau_{\mathrm{block}},\\
 \text{block}, & s_t \ge \tau_{\mathrm{block}}.
 \end{cases}
\end{equation}
...
\begin{equation}
U=1.6C_a+0.8A_b+0.25B_a-3.2B_b,
\end{equation}
...
\begin{equation}
\Delta_{\mathrm{decoy}}(t) = \| s_t^{\mathrm{live}} - s_t^{\mathrm{decoy}, k} \|_2.
\end{equation}
...
\begin{equation}
 K_{\mathrm{epoch}} = \operatorname{KDF}(K_{\mathrm{master}}, \text{epoch} \parallel p),
\end{equation}
\begin{equation}
 T_t = \operatorname{HMAC}_{K_{\mathrm{epoch}}}\bigl(\operatorname{Ser}(u,r,d,o,v,n,t_i,t_e,p)\bigr).
\end{equation}
...
\IF{$I_t$ is invalid or $\mathsf{CheckFreshness}(I_t) = \mathsf{False}$}
...
\IF{$s_t < \tau_{\mathrm{clear}}$}
    \STATE $K_{\mathrm{epoch}} \gets \operatorname{KDF}(K_{\mathrm{master}}, \text{epoch} \parallel p)$
    \STATE $T \gets \operatorname{HMAC}_{K_{\mathrm{epoch}}}(u\parallel r\parallel d\parallel o\parallel v\parallel n\parallel t_i\parallel t_e\parallel p)$
    \STATE \textbf{return} $\mathsf{Approve}$ with capability $T$
\ELSIF{$s_t < \tau_{\mathrm{block}}$}
    \STATE $\mathcal{S}_{\mathrm{decoy}}^{(k)} \gets \operatorname{InitDecoySession}(k, c_t)$
...
\ENDIF
\STATE $T' \gets \operatorname{HMAC}_{K_{\mathrm{epoch}}}(u\parallel r\parallel d\parallel o\parallel v\parallel n\parallel t_i\parallel t_e\parallel p)$
\IF{$\operatorname{CT-Compare}(T', T) \neq \mathsf{Equal}$}
    \STATE \textbf{return} $\mathsf{Reject}(\text{"Capability Authentication Mismatch"})$
\ENDIF
\STATE $\mathsf{AtomicAdd}(N_{\mathrm{used}}, n)$
\STATE $\mathsf{ForwardWriteToPLC}(d, o, v)$
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Seed} & \textbf{Accuracy} & \textbf{Macro-F1} & \textbf{FPR} & \textbf{Simulated Containment} \\
\midrule
